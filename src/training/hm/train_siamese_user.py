"""
src/training/hm/train_siamese_hm_user.py

Siamese baseline — cùng bài toán với GNN:
    score(user_u, item_i) → "user này có mua item này không?"

Khác biệt so với GNN:
    - KHÔNG dùng graph / edge_index
    - User embedding = mean-pool CLIP embeddings của các items đã mua trong train
    - Item embedding = CLIP embedding (qua SiameseProjector)
    - Loss: BPR (giống GNN)
    - Eval: sigmoid(user_emb · item_emb) (giống GNN)

→ Kết quả so sánh trực tiếp được với LightGCN / GraphSAGE / NGCF.

Fixes đã áp dụng:
    1. user_hist_emb được cache ra disk (precompute_user_emb.py), chỉ tính 1 lần
    2. user_hist_emb + item_feat chuyển sang torch shared memory tensor
       → DataLoader workers không crash, không copy, nhanh hơn
    3. persistent_workers=True + prefetch_factor=4
    4. Bug fix: `path` → `result_path` trong build hàm cũ (đã bỏ)
"""
...
def to_shared_tensor(arr: np.ndarray) -> torch.Tensor:
    """
    Chuyển numpy array sang torch Tensor trong POSIX shared memory.
    Tất cả DataLoader worker processes đọc chung vùng nhớ này:
      - Không copy qua fork  → tiết kiệm RAM
      - Không crash collate  → fix RuntimeError trong worker
...
class SiameseUserItemModel(nn.Module):
    """
    Dùng chung một SiameseProjector cho cả user lẫn item.
      user_emb(u) = projector( mean_pool_clip_history(u) )
      item_emb(i) = projector( clip(i) )
      score(u,i)  = user_emb · item_emb   (dot-product, cả hai đã L2-norm)
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Datasets  (nhận torch.Tensor shared memory thay vì numpy array)
# ─────────────────────────────────────────────────────────────────────────────

class SiameseTrainDataset(Dataset):
    """
    BPR format: (user_hist_emb, pos_item_feat, neg_item_feat).
    Negative được sample random mỗi epoch (resample()).
    user_hist_emb và item_feat là shared memory Tensor → worker-safe.
    """
    def __init__(self, user_arr, item_arr, user_hist_emb, item_feat, n_items):
        assert isinstance(user_hist_emb, torch.Tensor), \
            "user_hist_emb phải là torch.Tensor shared. Dùng to_shared_tensor()."
        assert isinstance(item_feat, torch.Tensor), \
            "item_feat phải là torch.Tensor shared. Dùng to_shared_tensor()."
...
class SiameseEvalDataset(Dataset):
    """
    Binary eval: (user_hist_emb, item_feat, label).
    neg_arr: pre-sampled negatives — tránh load adj_csr (~1 GB).
    False-negative rate < 0.02% (27M known / 105k items / 1.36M users).
    """
...
class HMSiameseTrainer:
    ...
    def train(self, evaluator=None, dataset: str = "hm"):
        ...
        #    Chạy trước để cache: python -m src.training.hm.precompute_user_emb --feature <feature>
        ...
        #    → DataLoader workers đọc chung vùng nhớ, không crash collate_fn
        ...
        # 8. Eval datasets — pre-sampled negatives, KHÔNG dùng adj_csr (~1 GB)
        ...
        train_loader = DataLoader(
            ...
            num_workers=_NUM_WORKERS,
            pin_memory=True,
            persistent_workers=True,   # giữ worker processes giữa các epoch
            prefetch_factor=4,         # prefetch batch tiếp theo
        )
