...
def build_positive_pairs(user_arr, item_arr, time_arr):
    print("[PAIRS] Building positive pairs (numpy)...")

    # sort theo user rồi time
    idx = np.lexsort((time_arr, user_arr))
    user_sorted = user_arr[idx]
    item_sorted = item_arr[idx]

    # lấy những vị trí mà user[k] == user[k+1]
    mask = user_sorted[:-1] == user_sorted[1:]

    anchors   = item_sorted[:-1][mask]
    positives = item_sorted[1:][mask]

    # sample bớt
    max_pairs = 5_000_000
    if len(anchors) > max_pairs:
        print(f"[PAIRS] sampling {max_pairs:,} from {len(anchors):,}")
        sel = np.random.choice(len(anchors), max_pairs, replace=False)
        anchors   = anchors[sel]
        positives = positives[sel]

    print(f"[PAIRS] positive pairs: {len(anchors):,}")
    return anchors.astype(np.int32), positives.astype(np.int32)
